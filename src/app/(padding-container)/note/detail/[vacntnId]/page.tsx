...
export default function VaccineDetailPage() {
    ...
    const [vaccineDetail, setVaccineDetail] = useState<VaccineDetail | null>(
        null
    );
    ...
    const handleConfirm = async () => {
        try {
            ...
            const body = {
                ...
                vacntnNo: vaccineDetail?.vacntnCnt,
                hsptlNo: 1,
                hsptlDrctr: '병원명',
            ...
            console.error(
                'Error occurred while creating vaccination record:',
                error
            );
        }
    };
    ...
    return (
        <>
            <Header title="예방접종 기록" />
            <div className={styles.container}>
                <h1 className={styles.title}>{vaccineDetail?.vacntnIctsd}</h1>
                <Spacer height={36} />
                <div className={styles.vacccineStatus}>
                    <div className={styles.vacccineStatus_Count}>
                        <Label
                            text={`완료 ${vaccineDetail?.vacntnOdr}`}
                            css="completed"
                        />
                        <div className="divider"></div>
                        <Label
                            text={`미접종 ${vaccineDetail?.vacntnCnt}`}
                            css="incomplete"
                        />
                    </div>
                    <div className="horizonFlexbox align-center gap-8">
                        {vaccineDetail &&
                            Array.from({
                                length: vaccineDetail.vacntnCnt,
                            }).map((_, index) => (
                                <span
                                    key={`circle-${index}`}
                                    className={`${styles.circle} ${
                                        index < vaccineDetail.vacntnOdr
