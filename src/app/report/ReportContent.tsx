...
import RecordGraph from "@/components/metrics/RecordGraph";
  ...
  return (
    <Box>
        ...
        <Box px={16} ref={captureRef}>
          ...
          {profile && (
            ...
            >
              <Flex justify="space-between" mb="md">
                <Stack gap="md">
                  <ProfileMetrics
                    label={`${getFormatDate(profile.birthDate)?.substring(
                      0,
                      10
                    )} 출생`}
                    value={profile.name}
              ...
              <Flex align="center" justify="space-between">
                <ProfileMetrics label="몸무게" value={`${profile.weight}kg`} />
                <ProfileMetrics label="키" value={`${profile.height}cm`} />
                <ProfileMetrics
                  label="머리 둘레"
          ...
          <Box mt="xl">
            ...
            {vaccineData && vaccineData.length === 0 ? (
              ...
              >
                {vaccineData.map((vaccine: VaccinationRecord) => (
                  <Box
                    display="flex"
                    style={{
                      flexDirection: "column",
                    ...
                    key={vaccine.id}
                  >
                    <Text c={theme.other.fontColors.sub3} fz="md" fw={500}>
                      {getFormatDate(vaccine.vaccinationDate)}
                    </Text>
                    <Box
                      display="flex"
                      style={{
                      ...
                      pb={theme.spacing.lg}
                    >
                      <Text c="#000000" fz={theme.fontSizes.mdLg} fw={600}>
                        {vaccine.vaccineName}
                      </Text>
            ...
            {categoryRecords.length === 0 ? (
              ...
              <Box display="flex" my="12 40" style={{ gap: 4 }}>
                {categoryRecords
                  .filter(
                    (item: CategoryItem) =>
                      Array.isArray(item.behavior) && item.behavior.length > 0
                  )
      ...
      <Button
        ...
        style={{
          position: "fixed",
          bottom: isReactNativeWebView ? 100 : 100,
          right: 20,
          padding: 0,
          zIndex: 1000,
        }}
