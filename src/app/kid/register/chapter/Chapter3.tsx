...
const Chapter3: React.FC<ChapterProps> = ({ onNext }) => {
    ...
    const validateBirthDate = (value: string) => {
        const regex = /^\d{8}$/;
        if (!regex.test(value)) {
            setErrors(prev => ({ ...prev, birthDate: "YYYYMMDD 형식으로 입력해주세요 (예: 20240101)" }));
            return false;
        }
    ...
    const handleBirthDateChange = (value: string) => {
        ...
        if (numbersOnly.length > 0 && numbersOnly.length !== 8) {
            setErrors(prev => ({ ...prev, birthDate: "20241212 와 같이 입력해주세요" }));
            return;
        }
    ...
    const handleWeightChange = (value: string) => {
        ...
        if (!value) {
            ...
            setErrors(prev => ({ ...prev, weight: "올바른 형식으로 입력해주세요" }));
        } else {
    ...
    const handleHeightChange = (value: string) => {
        ...
        if (!value) {
            ...
            setErrors(prev => ({ ...prev, height: "올바른 형식으로 입력해주세요" }));
        } else {
