...
const Chapter5: React.FC<ChapterProps> = ({ onNext }) => {
  ...
  // 알레르기 입력
  const handleAllergicKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    // 한글 입력 중이면 무시
    if (e.nativeEvent.isComposing) {
      return;
    }
...
진료 기록을 알려주세요"
      ...
      <div className={`${styles.listInputArea} mt-48 `}>
        <Label text="과거에 진단받은 질환이 있나요?" css="profileDesc" />
      ...
      <div className="mt-20 verticalFlexbox gap-8 mb-48">
        {symptomList.map((symptom, index) => (
          <div key={index} className={styles.symptomList}>
            ...
            <XIcon color="#222222" onClick={() => removeSymptom(index)} />
          </div>
        ))}
      </div>
      ...
      <div className="mt-20 verticalFlexbox gap-8 mb-120">
        {allergicList.map((allergic, index) => (
          <div key={index} className={styles.symptomList}>
            ...
            <XIcon color="#222222" onClick={() => removeAllergic(index)} />
          </div>
        ))}
      </div>
